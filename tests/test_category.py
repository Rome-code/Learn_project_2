...
def test_add_product_positive() -> None:
...
def test_add_product_negative() -> None:
    """Проверка вывода ошибки при добавлении обьекта, не входящего в Product"""
    product1 = Product("Samsung Galaxy S23 Ultra", "256GB, Серый цвет, 200MP камера", 180000.0, 5)
    product2 = Product("Iphone 15", "512GB, Gray space", 210000.0, 8)
    product3 = Product("Xiaomi Redmi Note 11", "1024GB, Синий", 31000.0, 14)
    ...
    # Проверка, что исключение выбрасывается при неправильном типе
    with pytest.raises(TypeError, match="Объект Не товар недопустим.Можно добавлять только объекты класса Product."):
        category.add_product("Не товар")
